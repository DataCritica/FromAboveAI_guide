...
rf_model <- caret::train(x = trainDat[,(5:ncol(trn)-1)], y = as.factor(as.integer(as.factor(trainDat$annotation_class))),
                         method = "rf", metric="Accuracy", trainControl = tc, tuneGrid = rf.grid)

# # Train the neural network model
# nnet_model <- caret::train(x = trainDat[,(5:ncol(trn)-1)], y = as.factor(as.integer(as.factor(trainDat$annotation_class))),
#                            method = "nnet", metric="Accuracy", trainControl = tc, tuneGrid = nnet.grid)

# Apply the random forest model to the Sentinel-2 data
rf_prediction = raster::predict(satellite_data, model=rf_model)
# Convert the evaluation data into a spatial object using the X and Y coordinates and extract predicted values
eva.sp = SpatialPointsDataFrame(coords = cbind(testDat$x, testDat$y), data = testDat, 
...
clas_sup <- superClass(satellite_data,trainData = labels_crs, responseCol = "default",
                       model="mlc")
```

```{r load model data ,echo = FALSE}

load("process_data/FromAbovemodel.RData")
