...
# model_data[c(5,6,8:11)] <- NULL #We get rid of non-informative columns
# 
# model_data <- unnest_wider(model_data, centroids) #make the "centroids" column, a list, into two columns in a data.frame
# names(model_data)[6] <- "x"
# names(model_data)[7] <- "y"
...
```{r predict}
#planet_data <- raster::raster("colombia2_analytic.tif")
# prediction <- predict(satellite_data, model, drop_dimensions=TRUE)  #need to reactivate

# plot(prediction), col = sf.colors(nclus, categorical=TRUE), reset = FALSE)
...
#data_labels <- as.data.frame(aggregate(satellite_data, labels_crs, FUN = mean)) To run this code, take the # out of the line
#data_labels <- data_labels %>% pivot_wider(names_from = band, values_from = colombia2_analytic.tif)
#labels_crs <- left_join(labels_crs, data_labels)
#labels_crs[c(3:11)] <- NULL #Get rid of non informative columns
#names(labels_crs) <- c("id","cover_class", "band1", "band2", "band3", "band4", "band5", "geometry")
#save(labels_crs, file="labels_crs.RData")
#Here, we load the result of the previous code to avoid running it every time and we see that the data is all merged
#load("labels_crs.RData")
#head(labels_crs)

```

Now we are ready to train our model with the merged data of the raster image + labelling polygons we gained from the collective annotation. First we transform the sf object into a simpler dataframe
...
# 
# 
# clas_nosup <- unsuperClass(satellite_data, nSamples = 200, nClasses = 6, nStarts = 5)
# class(clas_nosup)
